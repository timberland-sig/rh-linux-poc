...
install_edk2() {
    ...
    if [ ! -d edk2 ]; then
        ...
        git clone -b timberland_1_0_1-john-nospdk git@github.com:johnmeneghini/edk2.git
        pushd edk2
    ...
    make -C BaseTools
    source edksetup.sh
    build -t GCC -a X64 -p OvmfPkg/OvmfPkgX64.dsc -D NETWORK_PXE_BOOT_ENABLE=FALSE 2>&1 | tee -a build.log
